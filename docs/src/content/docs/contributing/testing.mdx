...
<Tabs syncKey="language">
  ...
  <TabItem label="Rust" icon="seti:rust">
    ...
    cargo test model                      # Filter by name
...
| Rust | `runtime/rust/agentschema/` | `Cargo.toml` is preserved |

:::caution
Test files matching `test_load_*.py`, `*ConversionTests.cs`, `*.test.ts`, `*_test.go`, or `*_test.rs` patterns ARE regenerated. Add custom tests with different naming patterns.
:::
