...
```bash
# Generate all default targets (Python, C#, TypeScript, Go, Rust)
...
| `--targets <list>` | `-t` | Comma-separated targets | `python,csharp,typescript,go,rust` |
...
interface GenerateOptions {
  ...
  targets?: ("python" | "csharp" | "typescript" | "go" | "rust" | "markdown")[];
...
| `rust` | Rust 1.75+ | Structs | serde, serde_json, serde_yaml |
