...
// TestBindingRoundtripYAML tests YAML load -> save -> load produces equivalent data
func TestBindingRoundtripYAML(t *testing.T) {
	yamlData := `
name: my-tool
input: input-variable
...
// TestBindingFromStringYAML tests loading Binding from string via YAML
func TestBindingFromStringYAML(t *testing.T) {
	yamlData := `"example"`
	var data interface{}
	if err := yaml.Unmarshal([]byte(yamlData), &data); err != nil {
		t.Fatalf("Failed to parse YAML: %v", err)
	...
	instance, err := agentschema.LoadBinding(data, ctx)
	if err != nil {
		t.Fatalf("Failed to load Binding from string YAML: %v", err)
	}
	if instance.Input != "example" {
		t.Errorf(`Expected Input to be "example", got %v`, instance.Input)
	}
...
// TestBindingLoadFromMap tests loading Binding from an empty map
func TestBindingLoadFromMap(t *testing.T) {
	data := make(map[string]interface{})
	ctx := agentschema.NewLoadContext()
	instance, err := agentschema.LoadBinding(data, ctx)
	if err != nil {
		t.Fatalf("Failed to load Binding from map: %v", err)
	}
	_ = instance // Load succeeded
}

// TestBindingSaveToMap tests saving Binding to a map
func TestBindingSaveToMap(t *testing.T) {
	instance := agentschema.Binding{}
