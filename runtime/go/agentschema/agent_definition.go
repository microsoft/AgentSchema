...
func LoadAgentDefinition(data interface{}, ctx *LoadContext) (AgentDefinition, error) {
	...
	if m, ok := data.(map[string]interface{}); ok {
		...
		if val, ok := m["displayName"]; ok && val != nil {v := val.(string)
		...
		if val, ok := m["inputSchema"]; ok && val != nil {
			if m, ok := val.(map[string]interface{}); ok {
				loaded, _ := LoadPropertySchema(m, ctx)result.Inputschema = &loaded}
		}
		if val, ok := m["outputSchema"]; ok && val != nil {
			if m, ok := val.(map[string]interface{}); ok {
				loaded, _ := LoadPropertySchema(m, ctx)result.Outputschema = &loaded}
		}
	}
...
func (obj *AgentDefinition) Save(ctx *SaveContext) map[string]interface{} {
	...
	if obj.Displayname != nil {result["displayName"] = *obj.Displayname}
	if obj.Description != nil {result["description"] = *obj.Description}
	if obj.Metadata != nil {
		result["metadata"] = obj.Metadata
	}
	if obj.Inputschema != nil {result["inputSchema"] = obj.Inputschema.Save(ctx)}
	if obj.Outputschema != nil {result["outputSchema"] = obj.Outputschema.Save(ctx)}
...
func LoadPromptAgent(data interface{}, ctx *LoadContext) (PromptAgent, error) {
	...
	if m, ok := data.(map[string]interface{}); ok {
		...
		if val, ok := m["model"]; ok && val != nil {
			if m, ok := val.(map[string]interface{}); ok {
				loaded, _ := LoadModel(m, ctx)result.Model = loaded}
		}
		...
		if val, ok := m["template"]; ok && val != nil {
			if m, ok := val.(map[string]interface{}); ok {
				loaded, _ := LoadTemplate(m, ctx)result.Template = &loaded}
		}
		if val, ok := m["instructions"]; ok && val != nil {v := val.(string)
			result.Instructions = &v}
		if val, ok := m["additionalInstructions"]; ok && val != nil {v := val.(string)
...
func (obj *PromptAgent) Save(ctx *SaveContext) map[string]interface{} {
	...
	if obj.Template != nil {result["template"] = obj.Template.Save(ctx)}
	if obj.Instructions != nil {result["instructions"] = *obj.Instructions}
	if obj.Additionalinstructions != nil {result["additionalInstructions"] = *obj.Additionalinstructions}
