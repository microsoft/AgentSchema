...
// TestMcpToolRoundtripYAML tests YAML load -> save -> load produces equivalent data
func TestMcpToolRoundtripYAML(t *testing.T) {
	yamlData := `
kind: mcp
connection:
  kind: reference
...
// TestMcpToolLoadFromMap tests loading McpTool from an empty map
func TestMcpToolLoadFromMap(t *testing.T) {
	data := make(map[string]interface{})
	ctx := agentschema.NewLoadContext()
	instance, err := agentschema.LoadMcpTool(data, ctx)
	if err != nil {
		t.Fatalf("Failed to load McpTool from map: %v", err)
	}
	_ = instance // Load succeeded
}

// TestMcpToolSaveToMap tests saving McpTool to a map
func TestMcpToolSaveToMap(t *testing.T) {
	instance := agentschema.McpTool{}
