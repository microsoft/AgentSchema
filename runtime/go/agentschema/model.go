...
func LoadModel(data interface{}, ctx *LoadContext) (Model, error) {
	...
	if m, ok := data.(map[string]interface{}); ok {
		if val, ok := m["id"]; ok && val != nil {result.Id = val.(string)}
		if val, ok := m["provider"]; ok && val != nil {v := val.(string)
			result.Provider = &v}
		if val, ok := m["apiType"]; ok && val != nil {v := val.(string)
			result.Apitype = &v}
		...
		if val, ok := m["options"]; ok && val != nil {
			if m, ok := val.(map[string]interface{}); ok {
				loaded, _ := LoadModelOptions(m, ctx)result.Options = &loaded}
		}
	}
...
func (obj *Model) Save(ctx *SaveContext) map[string]interface{} {
	...
	result["id"] = obj.Id
	if obj.Provider != nil {result["provider"] = *obj.Provider}
	if obj.Apitype != nil {result["apiType"] = *obj.Apitype}
	if obj.Connection != nil {result["connection"] = obj.Connection.Save(ctx)}
	if obj.Options != nil {result["options"] = obj.Options.Save(ctx)}
