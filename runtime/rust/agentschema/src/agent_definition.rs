...
use crate::code_configuration::CodeConfiguration;

use crate::container_resources::ContainerResources;
...
impl Workflow {
    ...
    pub fn load_from_value(value: &serde_json::Value) -> Self {
        Self {
            kind: value.get("kind").and_then(|v| v.as_str()).unwrap_or_default().to_string(),
            trigger: value.get("trigger").cloned().unwrap_or(serde_json::Value::Null),
        }
    }
...
impl ContainerAgent {
    ...
    pub fn load_from_value(value: &serde_json::Value) -> Self {
        Self {
            kind: value.get("kind").and_then(|v| v.as_str()).unwrap_or_default().to_string(),
            protocols: value.get("protocols").cloned().unwrap_or(serde_json::Value::Null),
            image: value.get("image").and_then(|v| v.as_str()).map(|s| s.to_string()),
            dockerfile_path: value.get("dockerfilePath").and_then(|v| v.as_str()).map(|s| s.to_string()),
            resources: value.get("resources").filter(|v| v.is_object() || v.is_array()).map(ContainerResources::load_from_value).unwrap_or_default(),
            environment_variables: value.get("environmentVariables").cloned().unwrap_or(serde_json::Value::Null),
            code_configuration: value.get("codeConfiguration").filter(|v| v.is_object() || v.is_array()).map(CodeConfiguration::load_from_value),
        }
    }
    ...
    pub fn to_value(&self) -> serde_json::Value {
        ...
        if let Some(ref val) = self.dockerfile_path {
            result.insert("dockerfilePath".to_string(), serde_json::Value::String(val.clone()));
        }
        {
            let nested = self.resources.to_value();
            if !nested.is_null() {
                result.insert("resources".to_string(), nested);
            }
        }
        if !self.environment_variables.is_null() {
            result.insert("environmentVariables".to_string(), self.environment_variables.clone());
        }
        if let Some(ref val) = self.code_configuration {
            let nested = val.to_value();
            if !nested.is_null() {
                result.insert("codeConfiguration".to_string(), nested);
            }
        }
        serde_json::Value::Object(result)
    }
    ...
    pub fn as_protocols(&self) -> Option<Vec<ProtocolVersionRecord>> {
        match &self.protocols {
            serde_json::Value::Array(arr) => {
                Some(arr.iter().map(ProtocolVersionRecord::load_from_value).collect())
            }
            serde_json::Value::Object(obj) => {
    ...
    pub fn as_environment_variables(&self) -> Option<Vec<EnvironmentVariable>> {
        match &self.environment_variables {
            serde_json::Value::Array(arr) => {
                Some(arr.iter().map(EnvironmentVariable::load_from_value).collect())
            }
            serde_json::Value::Object(obj) => {
