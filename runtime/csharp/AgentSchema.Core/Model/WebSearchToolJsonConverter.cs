...
public class WebSearchToolJsonConverter: JsonConverter<WebSearchTool>
{
    public override WebSearchTool Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
    {
        ...
        {
            throw new JsonException("Cannot convert null value to WebSearchTool.");
        }
        else if (reader.TokenType != JsonTokenType.StartObject)
        {
            throw new JsonException($"Unexpected JSON token when parsing WebSearchTool: {reader.TokenType}");
        }
    ...
    public override void Write(Utf8JsonWriter writer, WebSearchTool value, JsonSerializerOptions options)
    {
